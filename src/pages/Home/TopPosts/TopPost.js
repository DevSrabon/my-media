...
const TopPost = ({ post }) => {
	const { description, _id, image, date, photoURL, displayName } = post;
...
const { data: comments = [], refetch } = useQuery({
	...
	queryFn: async () => {
		const res = await fetch(
			`${process.env.REACT_APP_API_URL}/allComments/${user?.email}`
		);
...
console.log(comments);
	...
	// fetch(`${process.env.REACT_APP_API_URL}/allComments/${user?.email}`, {
	// 	method: "GET",
	// 	headers: {
	// 		"content-type": "application/json",
	// 	},
	...
	return (
		<div className="rounded-md shadow-md sm:w-96 mb-5 dark:bg-gray-900 dark:text-gray-100">
				...
				<form onSubmit={handleSubmit} className="space-y-3">
					...
					<>
						{comments.map((comment) => (
							<div className="p-5 bg-slate-400 rounded-md text-white">
